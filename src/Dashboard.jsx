...
const milestones = [
  { goal: 50, reward: "🎨 Graphic Design Course + 10,000 Coins", icon: "💰" },
  { goal: 100, reward: "💻 Web Dev Course + 20,000 Coins + Laptop", icon: "🚀" },
  { goal: 200, reward: "🕋 Umrah Ticket + 30,000 Coins", icon: "✈️" },
  { goal: 400, reward: "🏍️ Honda CD 70 + 50,000 Coins", icon: "💵" },
  { goal: 700, reward: "🚗 Suzuki Alto + 1 Lac Coins + Dubai Trip", icon: "🏖️" },
];
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col lg:flex-row font-sans text-slate-900 antialiased overflow-x-hidden">
      ...
      <aside className="w-72 bg-white border-r hidden lg:flex flex-col p-8 sticky top-0 h-screen z-50">
        ...
        <nav className="space-y-3 flex-1">
          ...
          <button onClick={()=>setActiveTab('surveys')} className={`w-full text-left px-5 py-4 rounded-2xl font-bold text-sm transition-all ${activeTab==='surveys'?'bg-blue-600 text-white shadow-xl':'text-slate-400 hover:bg-slate-50'}`}>Earn Coins from surveys 🔥</button>
      ...
      <main className="flex-1 p-4 md:p-12 pb-28 lg:pb-12 max-w-full overflow-x-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 bg-white p-5 md:p-7 rounded-4xl shadow-sm border border-slate-50">
          <div>
            <h2 className="text-xl md:text-3xl font-black text-slate-800 uppercase italic tracking-tighter leading-tight">
              {activeTab === 'dashboard' ? 'DASHBOARD' : activeTab === 'surveys' ? 'Earn Coins from surveys' : activeTab.toUpperCase()}
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            <FeatureLock title="My Wallet">
              <div className="bg-white p-6 md:p-8 rounded-4xl border border-slate-100 shadow-sm text-center h-full flex flex-col justify-between">
                <p className="text-slate-400 font-bold text-[9px] uppercase tracking-widest mb-1">Available Coins</p>
                <h3 className="text-2xl md:text-3xl font-black text-slate-800 font-mono italic">{userData.walletBalance} FC Coins</h3>
                <button onClick={()=>setActiveModal('withdraw')} className="mt-5 w-full bg-slate-900 text-white py-3.5 rounded-2xl font-black text-[9px] uppercase hover:bg-red-600 transition-all">Redeem Coins</button>
              </div>
            </FeatureLock>
            ...
            <FeatureLock title="Total Income">
              <div className="bg-white p-6 md:p-8 rounded-4xl border border-slate-100 shadow-sm text-center flex flex-col justify-center h-full min-h-35">
                ...
                <h3 className="text-2xl md:text-3xl font-black text-emerald-500 font-mono italic">{userData.walletBalance + (userData.totalWithdraw || 0)} Coins</h3>
              </div>
            </FeatureLock>
        ...
        {activeTab === 'surveys' && (
          <FeatureLock 
            title="Survey Core" 
            subtitle="Activate account to earn Coins from surveys"
          ...
          >
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] md:rounded-[3rem] border border-slate-100 shadow-xl text-center max-w-2xl mx-auto">
              ...
              <h3 className="text-2xl md:text-3xl font-black text-slate-800 uppercase italic mb-3">Earn Coins from surveys</h3>
              <p className="text-slate-400 font-semibold text-[10px] uppercase tracking-wide mb-8 leading-relaxed">High-yield market research surveys. Earn up to <span className="text-indigo-600 font-black text-xs underline">500 Coins</span> per completion.</p>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-200 flex items-center justify-center p-3 md:p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-2xl overflow-y-auto max-h-[95vh] relative">
            <h2 className="text-xl md:text-2xl font-black mb-6 md:mb-8 uppercase text-center text-slate-800 italic tracking-tighter">{activeModal === 'upgrade' ? 'Subscription' : 'Redeem Coins'}</h2>
            ...
            {activeModal === 'upgrade' ? (
              ...
              <form onSubmit={handleWithdraw} className="space-y-3">
                <div className="bg-slate-50 p-5 rounded-2xl mb-2 text-center border border-dashed border-slate-200">
                   <p className="text-[8px] font-bold text-slate-400 uppercase mb-1 tracking-widest">Balance in Coins</p>
                   <p className="text-xl font-black italic text-emerald-600">{userData.walletBalance} FC Coins</p>
                   <p className="text-[7px] font-black text-slate-400 uppercase mt-1">10 Coins = 1 PKR</p>
                </div>
                <input type="number" placeholder="Coins to Redeem" className="w-full p-4 bg-white border border-slate-100 rounded-xl font-black text-center text-lg outline-none" onChange={(e)=>setWithdrawData({...withdrawData, amount: e.target.value})} required />
                ...
                <button type="submit" className="w-full bg-slate-900 text-white p-4 rounded-2xl font-black uppercase italic text-xs active:scale-95 transition-all">Redeem Now</button>
              </form>
            )}
