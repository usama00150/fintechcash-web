  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row font-sans relative">
      ...
      <div className="lg:hidden fixed bottom-4 left-1/2 -translate-x-1/2 w-[92%] bg-white border border-slate-100 flex justify-around p-3 z-50 shadow-2xl rounded-full">
        <button onClick={()=>setActiveTab('dashboard')} className={`px-3 py-1 font-black text-[9px] uppercase italic tracking-tighter transition-all ${activeTab==='dashboard'?'text-blue-600 border-b-2 border-blue-600':'text-gray-400'}`}>HOME</button>
        <button onClick={()=>setActiveTab('network')} className={`px-3 py-1 font-black text-[9px] uppercase italic tracking-tighter transition-all ${activeTab==='network'?'text-blue-600 border-b-2 border-blue-600':'text-gray-400'}`}>TEAM</button>
        <button onClick={()=>setActiveTab('ads')} className={`px-3 py-1 font-black text-[9px] uppercase italic tracking-tighter transition-all ${activeTab==='ads'?'text-blue-600 border-b-2 border-blue-600':'text-gray-400'}`}>ADS</button>
        <button onClick={()=>setActiveTab('rewards')} className={`px-3 py-1 font-black text-[9px] uppercase italic tracking-tighter transition-all ${activeTab==='rewards'?'text-blue-600 border-b-2 border-blue-600':'text-gray-400'}`}>GIFT</button>
        ...
        <div className={isLocked ? "blur-md pointer-events-none" : "animate-in fade-in duration-500"}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-3 bg-blue-600 p-6 rounded-4xl text-white shadow-xl mt-4 relative overflow-hidden group">
                  ...
                  <div className="flex flex-col md:flex-row gap-3">
                     <input readOnly value={`${window.location.origin}/signup?ref=${userData.uid}`} className="flex-1 bg-white/10 px-4 py-3 rounded-2xl text-[10px] font-mono outline-none border border-white/10" />
                     <button onClick={()=>{navigator.clipboard.writeText(`${window.location.origin}/signup?ref=${userData.uid}`); alert("Copied!")}} className="bg-white text-blue-700 px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all">Copy Link</button>
                  </div>
              </div>
            </div>
          )}
              ...
              {getAdReward() > 0 ? (
                <div className="grid grid-cols-1 gap-3">
                  {[1, 2, 3, 4, 5].map((num) => {
                    ...
                    return (
                      <div key={num} className={`p-6 rounded-4xl border flex justify-between items-center transition-all ${isWatched ? 'bg-gray-100 opacity-60' : 'bg-white shadow-sm border-blue-50'}`}>
                         <span className="font-black text-gray-700 text-sm italic uppercase">Advertisement #{num}</span>
                         {isWatched ? (
                           <span className="text-green-600 font-black text-[10px] uppercase italic">Completed ✅</span>
                         ) : (
                           <button onClick={() => handleAdComplete(num)} className="bg-blue-600 text-white px-6 py-2 rounded-2xl font-black text-[9px] uppercase shadow-md active:scale-95 transition-all">Watch & Earn Rs.{getAdReward()}</button>
                         )}
                      </div>
                    );
                  })}
            ...
            <div className="bg-white p-6 md:p-12 rounded-4xl shadow-sm border text-center">
               ...
               <div className="flex flex-col items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full pt-8 border-t border-gray-50">
                    {myTeam.map((m, i) => (
                      <div key={i} className="bg-gray-50 p-6 rounded-4xl border text-center relative hover:border-blue-300 hover:shadow-md transition-all">
          ...
          {activeTab === 'rewards' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in zoom-in duration-500">
              {milestones.map((m, i) => (
                <div key={i} className={`p-8 rounded-4xl border-2 flex flex-row justify-between items-center gap-6 transition-all transform hover:scale-[1.01] shadow-sm ${userData.referralCount >= m.goal ? 'bg-green-50 border-green-200' : 'bg-white border-slate-100 opacity-90'}`}>
                  <div className="flex items-center gap-6 text-left flex-1">
                    <span className="text-5xl drop-shadow-md">{m.icon}</span>
                    <div className="flex-1">
                      <h4 className="font-black text-gray-800 uppercase text-sm md:text-base leading-tight tracking-tighter">{m.reward}</h4>
                      <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1">Target: <span className="text-blue-600">{m.goal}</span> Referrals</p>
                      
                      <div className="w-full bg-gray-100 h-1.5 rounded-full mt-3 overflow-hidden">
                        <div 
                          className={`h-full transition-all duration-1000 ${userData.referralCount >= m.goal ? 'bg-green-500' : 'bg-blue-400'}`}
                          style={{ width: `${Math.min((userData.referralCount / m.goal) * 100, 100)}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
                  <span className={`font-black text-[9px] uppercase italic p-2 rounded-xl transition-all ${userData.referralCount >= m.goal ? 'bg-green-100 text-green-600 animate-bounce' : 'bg-gray-50 text-gray-300'}`}>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-200 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-[95%] md:max-w-md rounded-4xl p-6 md:p-10 shadow-2xl overflow-y-auto max-h-[90vh] animate-in zoom-in duration-300">
            ...
            {activeModal === 'upgrade' ? (
              <div className="space-y-4">
                <div className="bg-linear-to-br from-blue-700 to-indigo-900 p-6 md:p-8 rounded-4xl text-white shadow-xl text-center border border-white/20">
                  <p className="text-[9px] font-black opacity-60 mb-2 uppercase tracking-widest italic">MEEZAN BANK</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {plans.map(p => (
                    <div key={p.name} onClick={() => setSelectedPlan(p)} className={`p-3 border-2 rounded-3xl text-center cursor-pointer transition-all ${selectedPlan?.name === p.name ? 'border-blue-600 bg-blue-50' : 'border-gray-100'}`}>
                ...
                <input type="text" placeholder="Account Name / Number" className="w-full p-4 bg-gray-50 border rounded-3xl font-bold text-center text-sm outline-none focus:border-blue-600" onChange={(e)=>setSenderAccount(e.target.value)} />
                ...
                <button onClick={handleActivationRequest} className="w-full bg-blue-700 text-white p-4 rounded-3xl font-black uppercase shadow-xl italic tracking-tighter text-sm active:scale-95 transition-all">Submit Payment</button>
              </div>
            ) : (
              <form onSubmit={handleWithdraw} className="space-y-4">
                <input type="number" placeholder="Enter Amount (Min 500)" className="w-full p-4 bg-gray-50 border rounded-3xl font-black text-center text-xl outline-none focus:border-red-600" onChange={(e)=>setWithdrawData({...withdrawData, amount: e.target.value})} required />
                <select className="w-full p-4 bg-gray-50 border rounded-3xl font-bold text-center text-sm outline-none" onChange={(e)=>setWithdrawData({...withdrawData, method: e.target.value})} required>
                ...
                <button type="submit" className="w-full bg-red-600 text-white p-4 rounded-3xl font-black uppercase shadow-xl italic tracking-tighter text-sm active:scale-95 transition-all">Request Withdrawal</button>
              </form>
            )}
            <button onClick={()=>setActiveModal(null)} className="w-full mt-4 text-gray-400 font-bold text-[9px] uppercase tracking-widest text-center hover:text-gray-600 transition-colors">Close Window</button>
          </div>
        </div>
      )}
    </div>
